import { Coach, Category, Review, BlogArticle, Testimonial, BookingSession, DetectivePackage } from '../types';

export const MOCK_CATEGORIES: Category[] = [
  {
    id: 'dating-mindset',
    title: 'Dating & Confidence',
    description: 'Overcome anxiety, build magnetic charisma, and approach dating with authentic self-assurance.',
    iconName: 'HeartHandshake',
    coachesCount: 42,
    gradient: 'from-pink-500 to-rose-500'
  },
  {
    id: 'detective-verification',
    title: 'Detective & Verification Agency',
    description: '100% confidential pre-marital background checks, partner verification, & discreet loyalty investigations.',
    iconName: 'ShieldAlert',
    coachesCount: 18,
    gradient: 'from-amber-500 to-red-600'
  },
  {
    id: 'communication',
    title: 'Communication & Intimacy',
    description: 'Master conflict resolution, non-violent communication, and deep emotional vulnerability.',
    iconName: 'MessageSquareHeart',
    coachesCount: 58,
    gradient: 'from-indigo-500 to-purple-500'
  },
  {
    id: 'marriage-longterm',
    title: 'Marriage & Couples Counseling',
    description: 'Reignite passion, rebuild trust after turmoil, and create long-lasting partnership harmony with Gottman methods.',
    iconName: 'Sparkles',
    coachesCount: 36,
    gradient: 'from-violet-500 to-fuchsia-500'
  },
  {
    id: 'breakup-healing',
    title: 'Breakup & Divorce Recovery',
    description: 'Heal from heartache, regain your identity, and prepare emotionally for healthy new beginnings.',
    iconName: 'ShieldCheck',
    coachesCount: 29,
    gradient: 'from-cyan-500 to-blue-500'
  },
  {
    id: 'attachment-styles',
    title: 'Attachment Style Coaching',
    description: 'Understand Anxious/Avoidant triggers and shift towards secure, resilient relational attachment.',
    iconName: 'Compass',
    coachesCount: 31,
    gradient: 'from-emerald-500 to-teal-500'
  },
  {
    id: 'online-dating-profiles',
    title: 'Profile & Social Optimization',
    description: 'Craft high-converting dating profiles, messaging strategy, and first-date dynamics.',
    iconName: 'UserCheck',
    coachesCount: 24,
    gradient: 'from-amber-500 to-orange-500'
  }
];

export const MOCK_DETECTIVE_PACKAGES: DetectivePackage[] = [
  {
    id: 'pkg-basic',
    title: 'Pre-Dating Background Verification',
    price: 199,
    description: 'Discreet background check before committing or getting serious with an online match.',
    features: [
      'Identity & Employment Verification',
      'Public & Criminal Record Scan',
      'Social Media & Dating Profile Audit',
      'Confidential PDF Intelligence Report',
      '24-Hour Express Delivery'
    ]
  },
  {
    id: 'pkg-pro',
    title: 'Pre-Marital Comprehensive Audit',
    price: 499,
    isPopular: true,
    description: 'In-depth pre-wedding background investigation for families and couples to ensure total peace of mind.',
    features: [
      'Full Financial & Business Standing Check',
      'Past Marital & Legal Background Verification',
      'Discrete Field & Background Inquiry',
      'Social Conduct & Reputation Audit',
      '1-on-1 Strategy Session with Founder Prateek Mhaisane',
      '100% Encrypted & NDA Protected'
    ]
  },
  {
    id: 'pkg-elite',
    title: 'Partner Loyalty & Infidelity Investigation',
    price: 899,
    description: 'Discreet digital and field surveillance for suspected infidelity or marital breach.',
    features: [
      'Digital Footprint & Secret Account Discovery',
      'Discreet Physical & Field Surveillance Report',
      'Timeline & Location Log',
      'Admissible Evidence & Forensic Photo/Video Dossier',
      'Dedicated Private Investigator assigned by Prateek Mhaisane',
      'Direct Encrypted Hotline Access'
    ]
  }
];

export const MOCK_COACHES: Coach[] = [
  {
    id: 'coach-prateek',
    name: 'Prateek Mhaisane',
    title: 'Founder & Lead Dating, Relationship & Verification Strategist',
    avatar: '/images/prateek_user.png',
    coverImage: 'https://images.unsplash.com/photo-1517245386807-bb43f82c33c4?q=80&w=1200&auto=format&fit=crop',
    rating: 4.99,
    reviewsCount: 348,
    experienceYears: 10,
    hourlyRate: 150,
    languages: ['English', 'Hindi'],
    categories: ['dating-mindset', 'detective-verification', 'online-dating-profiles', 'communication', 'marriage-longterm'],
    bio: 'Hi, I am Prateek Mhaisane—founder of DateMentor. In addition to 1-on-1 dating strategy and Gottman relationship counseling, I oversee our 100% confidential Detective & Verification Agency division, empowering clients with absolute clarity, relationship safety, and emotional peace of mind.',
    certifications: [
      'Founder & Master Coach at DateMentor Platform',
      'Licensed Private Intelligence & Background Investigator Specialist',
      'Certified Gottman Method Relationship Educator',
      'ICF Professional Certified Coach (PCC)'
    ],
    skills: [
      'Pre-Marital Background Verification',
      'Dating Profile & Identity Audits',
      'Infidelity & Loyalty Guidance',
      'Social Confidence & Charisma',
      'Couples Conflict De-escalation'
    ],
    isVerified: true,
    isAvailableToday: true,
    location: 'India / Online Worldwide (+91 9737372183)',
    featured: true,
    stats: {
      clientsCoached: 1250,
      hoursLogged: 4800,
      successRate: 99
    },
    availableSlots: [
      { date: '2026-07-25', slots: ['09:00 AM', '11:30 AM', '02:00 PM', '05:00 PM'] },
      { date: '2026-07-26', slots: ['10:00 AM', '01:00 PM', '04:00 PM'] },
      { date: '2026-07-27', slots: ['09:30 AM', '03:00 PM', '06:00 PM'] }
    ]
  },
  {
    id: 'coach-detective-vikram',
    name: 'Vikramaditya R. (Lead Investigator)',
    title: 'Senior Intelligence Officer & Pre-Marital Audit Specialist',
    avatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?q=80&w=600&auto=format&fit=crop',
    coverImage: 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?q=80&w=1200&auto=format&fit=crop',
    rating: 4.96,
    reviewsCount: 112,
    experienceYears: 14,
    hourlyRate: 180,
    languages: ['English', 'Hindi', 'Marathi'],
    categories: ['detective-verification'],
    bio: 'Former intelligence analyst working under Prateek Mhaisane\'s DateMentor Detective Agency. Specialized in confidential pre-marital background checks, financial audits, and discreet partner verification.',
    certifications: ['Certified Fraud Examiner (CFE)', 'Licensed Private Investigator', 'Digital Forensics Specialist'],
    skills: ['Pre-Marital Background Checks', 'Digital Forensics', 'Discreet Field Audits', 'Asset & Legal Verification'],
    isVerified: true,
    isAvailableToday: true,
    location: 'Mumbai & Delhi / Global Remote',
    featured: true,
    stats: {
      clientsCoached: 490,
      hoursLogged: 2400,
      successRate: 98
    },
    availableSlots: [
      { date: '2026-07-25', slots: ['10:00 AM', '02:00 PM', '06:00 PM'] },
      { date: '2026-07-26', slots: ['11:00 AM', '03:00 PM'] }
    ]
  },
  {
    id: 'coach-elena',
    name: 'Dr. Elena Vance',
    title: 'Senior Associate Clinical Psychologist & Gottman Couples Specialist',
    avatar: 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?q=80&w=600&auto=format&fit=crop',
    coverImage: 'https://images.unsplash.com/photo-1522071820081-009f0129c71c?q=80&w=1200&auto=format&fit=crop',
    rating: 4.97,
    reviewsCount: 142,
    experienceYears: 12,
    hourlyRate: 140,
    languages: ['English', 'Spanish'],
    categories: ['attachment-styles', 'communication', 'marriage-longterm'],
    bio: 'Handpicked by Prateek Mhaisane for DateMentor, Dr. Vance brings clinical psychology expertise to help couples and singles understand hidden attachment wounds and rebuild marital intimacy.',
    certifications: ['Ph.D. in Clinical Psychology (Columbia)', 'Certified Gottman Practitioner'],
    skills: ['Attachment Repair', 'Conflict Resolution', 'Emotional Safety'],
    isVerified: true,
    isAvailableToday: true,
    location: 'New York, USA',
    featured: true,
    stats: {
      clientsCoached: 540,
      hoursLogged: 2100,
      successRate: 97
    },
    availableSlots: [
      { date: '2026-07-25', slots: ['10:00 AM', '01:00 PM', '04:30 PM'] },
      { date: '2026-07-26', slots: ['11:00 AM', '03:30 PM'] }
    ]
  }
];

export const MOCK_REVIEWS: Review[] = [
  {
    id: 'rev-1',
    userName: 'Samantha K.',
    userAvatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?q=80&w=200&auto=format&fit=crop',
    rating: 5,
    date: 'Yesterday',
    comment: 'Prateek Mhaisane completely revolutionized my dating approach. Also, his Detective Verification report saved my sister from a fraudulent match before her wedding. Incredible dual-coaching & protection platform!',
    coachId: 'coach-prateek',
    verifiedBooking: true
  },
  {
    id: 'rev-2',
    userName: 'Michael R.',
    userAvatar: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?q=80&w=200&auto=format&fit=crop',
    rating: 5,
    date: '3 days ago',
    comment: 'Coach Prateek Mhaisane gave me actionable communication tools for first dates and absolute peace of mind.',
    coachId: 'coach-prateek',
    verifiedBooking: true
  }
];

export const MOCK_TESTIMONIALS: Testimonial[] = [
  {
    id: 't-1',
    name: 'Jessica & Thomas',
    role: 'Engaged after 6 months of coaching with Prateek',
    avatar: 'https://images.unsplash.com/photo-1522529599102-193c0d76b5b6?q=80&w=300&auto=format&fit=crop',
    quote: 'Prateek Mhaisane\'s DateMentor platform is unmatched. He identified our recurring communication traps and handled our pre-marital background checks with total confidentiality.',
    rating: 5,
    coachName: 'Prateek Mhaisane (Lead Coach)'
  },
  {
    id: 't-2',
    name: 'Ananya & Rohan S.',
    role: 'Pre-Marital Audit Client',
    avatar: 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?q=80&w=300&auto=format&fit=crop',
    quote: 'Prateek\'s Detective Agency division provided a detailed 24-hour verification report before our alliance. 100% professional and confidential!',
    rating: 5,
    coachName: 'Vikramaditya (Lead Detective)'
  }
];

export const MOCK_BLOG_ARTICLES: BlogArticle[] = [
  {
    id: 'art-1',
    title: 'Pre-Marital Due Diligence: Why Modern Couples Rely on Prateek Mhaisane\'s Detective Agency',
    slug: 'pre-marital-due-diligence-prateek',
    category: 'Verification & Safety',
    readTime: '6 min read',
    date: 'July 25, 2026',
    author: 'Prateek Mhaisane',
    authorAvatar: '/images/prateek_user.png',
    image: 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?q=80&w=800&auto=format&fit=crop',
    summary: 'Learn how discreet background checks, legal verification, and loyalty audits protect families and individuals in online dating and arranged marriages.',
    featured: true,
    content: `In an era of online dating profiles and hasty arrangements, relationship safety is paramount. At DateMentor, I established our dedicated Detective & Background Verification Agency division to provide clients with absolute truth and emotional security.`
  }
];

export const MOCK_USER_SESSIONS: BookingSession[] = [
  {
    id: 'sess-101',
    coachId: 'coach-prateek',
    coachName: 'Prateek Mhaisane',
    coachAvatar: '/images/prateek_user.png',
    coachTitle: 'Founder & Lead Dating Coach',
    date: 'Tomorrow, 02:00 PM',
    timeSlot: '02:00 PM - 02:50 PM',
    sessionType: 'video',
    durationMinutes: 50,
    price: 150,
    discount: 30,
    totalPrice: 120,
    status: 'upcoming',
    meetingLink: 'https://meet.datementor.com/room-prateek-101',
    notes: '1-on-1 Strategy Session with Prateek Mhaisane: Profile & Dating Mindset'
  }
];

export const MOCK_FAQ = [
  {
    question: 'How does Prateek Mhaisane\'s Detective Agency division protect confidentiality?',
    answer: 'Every background investigation and loyalty audit is bound by strict Non-Disclosure Agreements (NDA). All intelligence reports are delivered via 256-bit encrypted PDF directly to your private email or WhatsApp (+91 9737372183).'
  },
  {
    question: 'Can I book 1-on-1 coaching sessions directly with Prateek Mhaisane?',
    answer: 'Yes! Prateek Mhaisane personally takes 1-on-1 coaching clients, couples, and pre-marital audit consultations. You can reserve time directly on his calendar.'
  }
];
